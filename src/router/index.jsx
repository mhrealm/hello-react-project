import { createBrowserRouter } from 'react-router-dom';
import { lazy, Suspense, createElement } from 'react';
// 页面组件导入
import MainLayout from '../App.jsx'; // 主布局组件

// 可复用的懒加载组件包装函数
const lazyLoadComponent = importFunc => {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      {createElement(lazy(importFunc))}
    </Suspense>
  );
};

// 路由配置数组
const routeConfig = [
  // 根路径重定向到首页
  {
    path: '/',
    element: <MainLayout />,
  },
  {
    path: 'question-answer-session',
    name: '复刻网易云营销活动',
    grade: '1', // 难度等级
    element: lazyLoadComponent(
      () => import('@/views/function/questionAnswerSession/index.jsx')
    ),
  },
  {
    path: 'images-lazy-loading',
    name: '图片懒加载',
    grade: '1', // 难度等级
    element: lazyLoadComponent(
      () => import('@/views/function/imagesLazyLoading/index.jsx')
    ),
  },
  {
    path: 'load-with-refresh',
    name: '上拉加载下拉刷新',
    grade: '2', // 难度等级
    element: lazyLoadComponent(
      () => import('@/views/function/loadAndRefresh/index.jsx')
    ),
  },
  {
    path: 'useState',
    name: 'useState',
    grade: '1', // 难度等级
    element: lazyLoadComponent(
      () => import('@/views/frame/useState/index.jsx')
    ),
  },
  {
    path: 'useCallback',
    name: 'useCallback',
    grade: '1',
    element: lazyLoadComponent(
      () => import('@/views/frame/useCallback/index.jsx')
    ),
  },
  {
    path: 'useContext',
    name: 'useContext',
    grade: '1',
    element: lazyLoadComponent(
      () => import('@/views/frame/useContext/index.jsx')
    ),
  },
  {
    path: 'useEffect',
    name: 'useEffect',
    grade: '1',
    element: lazyLoadComponent(
      () => import('@/views/frame/useEffect/index.jsx')
    ),
  },
  {
    path: 'useLayoutEffect',
    name: 'useLayoutEffect',
    grade: '1',
    element: lazyLoadComponent(
      () => import('@/views/frame/useLayoutEffect/index.jsx')
    ),
  },
  {
    path: 'useMemo',
    name: 'useMemo',
    grade: '1',
    element: lazyLoadComponent(() => import('@/views/frame/useMemo/index.jsx')),
  },
  {
    path: 'useReducer',
    name: 'useReducer',
    grade: '1',
    element: lazyLoadComponent(
      () => import('@/views/frame/useReducer/index.jsx')
    ),
  },
  {
    path: 'useRef',
    name: 'useRef',
    grade: '1',
    element: lazyLoadComponent(() => import('@/views/frame/useRef/index.jsx')),
  },
  {
    path: 'useImperativeHandle',
    name: 'useImperativeHandle',
    grade: '2',
    element: lazyLoadComponent(
      () => import('@/views/frame/useImperativeHandle/index.jsx')
    ),
  },
  {
    path: 'useTransition',
    name: 'useTransition',
    grade: '1',
    element: lazyLoadComponent(
      () => import('@/views/frame/useTransition/index.jsx')
    ),
  },
  {
    path: 'saveValue',
    name: '展示更新前后的值',
    grade: '1',
    element: lazyLoadComponent(
      () => import('@/views/frame/saveValue/index.jsx')
    ),
  },
  {
    path: 'customHooks',
    name: '自定义Hook',
    grade: '1',
    element: lazyLoadComponent(
      () => import('@/views/frame/customHooks/index.jsx')
    ),
  },
  {
    path: 'hoc',
    name: '高阶组件',
    grade: '1',
    element: lazyLoadComponent(() => import('@/views/frame/hoc/index.jsx')),
  },
  {
    path: 'cacheCom',
    name: '缓存组件',
    grade: '1',
    element: lazyLoadComponent(
      () => import('@/views/frame/cacheCom/index.jsx')
    ),
  },
  {
    path: 'devServer',
    name: '配置代理服务器',
    grade: '1',
    element: lazyLoadComponent(
      () => import('@/views/frame/devServer/index.jsx')
    ),
  },
  {
    path: 'reduxCase',
    name: 'redux初体验',
    grade: '1',
    element: lazyLoadComponent(
      () => import('@/views/frame/reduxCase/index.jsx')
    ),
  },
  {
    path: 'mobxCase',
    name: 'mobx初体验',
    grade: '1',
    element: lazyLoadComponent(
      () => import('@/views/frame/mobxCase/index.jsx')
    ),
  },
  {
    path: 'mix-blend-mode',
    name: '混合模式',
    grade: '2',
    element: lazyLoadComponent(
      () => import('@/views/animation/mixBlendMode/index.jsx')
    ),
  },
  {
    path: 'layouts',
    name: '布局',
    grade: '1',
    element: lazyLoadComponent(() => import('@/views/animation/layouts.jsx')),
  },
  {
    path: 'background-shake',
    name: '背景抖动',
    grade: '1',
    element: lazyLoadComponent(
      () => import('@/views/animation/backgroundShake/index.jsx')
    ),
  },
  {
    path: 'canvas',
    name: 'canvas',
    grade: '1',
    element: lazyLoadComponent(() => import('@/views/animation/canvas.jsx')),
  },
  {
    path: 'animation',
    name: '动画',
    grade: '1',
    element: lazyLoadComponent(() => import('@/views/animation/animation.jsx')),
  },
  {
    path: 'slide',
    name: '滑动删除列表',
    grade: '1',
    element: lazyLoadComponent(
      () => import('@/views/animation/slide/index.jsx')
    ),
  },
  {
    path: 'refresh',
    name: '刷新',
    grade: '1',
    element: lazyLoadComponent(
      () => import('@/views/animation/refresh/index.jsx')
    ),
  },
];

export { routeConfig };
let router = createBrowserRouter(routeConfig);
// 创建路由实例
export default router;
